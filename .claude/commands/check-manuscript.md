...
- **Omit categories with no genuine issues**: After verifying GROBID-flagged items against the PDF, omit any cross-reference category entirely if all flagged items turn out to be GROBID parsing artifacts (e.g., special characters in names, footnote citations GROBID missed, mathematical notation misread as citations). Do not include paragraphs that say "X flagged, 0 genuine" — these add noise without value. Only report categories where genuine issues remain after verification.
...
- **Retraction & Integrity Check** (metacheck-powered, if available): If metacheck results are available, add a sub-section after the Automated Cross-Reference Check reporting:
  ...
  - `ref_replication`: Any cited papers for which replication or reproduction studies exist in the FLoRA database (Informational severity). Present the original reference alongside the replication/reproduction reference. This helps authors cite relevant replication evidence. Note: this module uses DOIs enriched by `ref_doi_check` via the metacheck pipeline, so it checks references even if DOIs were not originally provided by the authors. **Contextual guidance**: Where possible, add a brief sentence about whether the replication is relevant to the manuscript's argument. For example: if the manuscript's claims rely on the cited finding, note that the authors may want to check whether the replication supports or challenges it; if the cited work is only mentioned in passing (e.g., as background), note that engagement with the replication may be less critical. This guidance is not always possible (e.g., when the replication result is unclear from the title alone), so only include it when you can say something useful.
